...
class DDQNAgent(object):
    ...
    def load_model(self):
        # Provide compatible custom objects for older saved models that reference
        # legacy names such as 'keras.metrics.mse'. Map them to the current
        # MeanSquaredError loss so the model can be deserialized.
        custom_objs = {
            'mse': tf.keras.losses.MeanSquaredError(),
            'mean_squared_error': tf.keras.losses.MeanSquaredError(),
            'keras.metrics.mse': tf.keras.losses.MeanSquaredError(),
        }
        self.brain_eval.model = load_model(self.model_file, custom_objects=custom_objs)
        self.brain_target.model = load_model(self.model_file, custom_objects=custom_objs)
